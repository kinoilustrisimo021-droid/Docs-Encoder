...
The app will:
  1. Scan every uploaded template and auto-detect all unique {{...}} placeholders
     (paragraphs, tables/nested tables, and headers/footers are all scanned).
  ...
  3. Show a Review screen with the values entered + a text preview of each
     document before generating anything.
  4. Generate the final .docx file(s) with ONLY the placeholder text replaced -
     everything else (fonts, bold, tables, spacing, letterhead, etc.) stays
     exactly as in the original template. If more than one file was uploaded,
     they are bundled into a single .zip for download.
...
# --------------------------------------------------------------------------
# Helpers: walk a .docx in true document order (paragraphs + tables, incl.
# nested tables and headers/footers) so we can find every placeholder and
# safely write back into the correct run.
# --------------------------------------------------------------------------
def iter_block_items(parent):
    """Yield Paragraph / Table objects in the order they appear in `parent`,
    where `parent` is a Document or a table _Cell."""
...
def iter_all_paragraphs(doc: Document):
    """Every paragraph in the document body, all tables (recursively,
    including nested tables), and every header/footer of every section."""

    def _walk(block_source):
        for block in iter_block_items(block_source):
...
def preview_text(file_bytes: bytes, mapping: dict, limit_chars: int = 4000) -> str:
    """Lightweight plain-text preview (document order) with values applied,
    for the Review step - not a pixel-perfect render, just for confirmation."""
    doc = Document(io.BytesIO(file_bytes))
    lines = []
    for p in iter_all_paragraphs(doc):
        text = "".join(r.text for r in p.runs)
        ...
        def repl(m):
            return str(mapping.get(m.group(1).strip(), m.group(0)))

        lines.append(PLACEHOLDER_PATTERN.sub(repl, text))
    joined = "\n".join(lines)
...
@dataclass
class AppState:
    step: str = "upload"          # upload -> fill -> review
...
st.caption(
    ...
    "review → download the completed document(s), formatting untouched."
)
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "3. **Review** the values\n"
        "4. **Download** the finished document(s)"
    )
...
# --- Step 3: Review ---------------------------------------------------------
    ...
    st.subheader("Review your entries")

    st.table(
        {
            "Field": [label_for(k) for k in state.placeholders],
            "Value": [state.values[k] for k in state.placeholders],
        }
    )

    st.subheader("Document preview")
    st.caption("Text preview only - the downloaded .docx keeps the original template's full formatting.")
    for t in state.templates:
        with st.expander(f"📄 {t['name']}", expanded=False):
            st.text(preview_text(t["bytes"], state.values))
    ...
    with col1:
        if st.button("← Edit fields"):
            ...
            st.rerun()
    with col2:
        generate = st.button("✅ Generate document(s)", type="primary")
...
# --- Step 4: Download --------------------------------------------------------
    ...
    st.divider()
    if st.button("🔄 Fill out another one"):
        reset_all()
        st.rerun()
